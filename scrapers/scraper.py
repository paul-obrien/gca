...
sports = { "Acrobatics & Tumbling" : constants.ACROBATICS,
           ...
           "Cross Country/Track & Field (Men's and Women's)" : [constants.MENS_CROSS_COUNTRY, constants.WOMENS_CROSS_COUNTRY, constants.MENS_TRACK_FIELD, constants.WOMENS_TRACK_FIELD],
           "Dance" : constants.DANCE,
           ...
           "Ice Hockey" : constants.MENS_ICE_HOCKEY,
           "Lacrosse, Men's" : constants.MENS_LACROSSE,
           "Lacrosse, Women's" : constants.WOMENS_LACROSSE,
           ...
           "Spirit Squads" : constants.CHEERLEADING,
           ...
           "Tennis (Men's & Women's)" : [constants.MENS_TENNIS, constants.WOMENS_TENNIS],
...
def scrape_asp_site(college_name, sports, fields=["name", "title", "email", "phone"], custom_params=None):
    ...
    for line in script:
        params = line.split(", ")
        sport_name = strip_string(line.split("',")[0][9:].replace("\\'", "'"))
        index = int(params[3]) if params[3].isdigit() else int(params[4])
        if sport_name in sports:
            indices[sport_name] = [index - i, -1]
        if previous_sport and previous_sport in indices:
            indices[previous_sport][1] = (index - i)
